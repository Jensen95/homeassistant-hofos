 ...
 */
async function fetchHoforDataInternal(options) {
    const { startDate = new Date(new Date().setFullYear(new Date().getFullYear() - 1)), endDate = new Date(), kundenummer = process.env.HOFOR_KUNDENUMMER || '', bsKundenummer = process.env.HOFOR_BS_KUNDENUMMER || '', headless = true, } = options;
    ...
    try {
        ...
        const csvData = (await csvResponse.json()).data;
        // Parse CSV data
 ...
 */
export const fetchHoforData = async (options = {}) => {
