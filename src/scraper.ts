...
export class HoforScraper {
  ...
  constructor(
    private readonly credentials: HoforCredentials,
   ...
   */
  private async extractConsumption(): Promise<ConsumptionData | null> {
    if (!this.page) {
      throw new ScraperError('Browser not initialized', 'BROWSER_NOT_INITIALIZED');
    }
    ...
    try {
      ...
      const consumptionText = await this.page.textContent(
        '.consumption-value, [data-consumption-value]'
      );
      ...
      const consumptionValue = parseFloat(
        consumptionText.replace(/[^\d.,]/g, '').replace(',', '.')
      );

      if (isNaN(consumptionValue)) {
        throw new ScraperError(
          `Invalid consumption value: ${consumptionText}`,
          'INVALID_CONSUMPTION_VALUE'
        );
      }
   ...
   */
  private async extractPricePerM3(): Promise<PriceData | null> {
    if (!this.page) {
      throw new ScraperError('Browser not initialized', 'BROWSER_NOT_INITIALIZED');
    }
